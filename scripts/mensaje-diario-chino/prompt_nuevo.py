...
prompt = """You are an assistant that simplifies and rewrites text into Chinese using a restricted vocabulary.
...
Source text (Wikipedia extract):
{articulo}
"""

def generar_prompt_nuevo(fecha, articulo):
    articulo_safe = articulo.replace("{", "{{").replace("}", "}}")
